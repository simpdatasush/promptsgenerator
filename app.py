...
def generate_image_with_imagen(prompt):
    ...
    try:
        # Define an async function to make the request and get JSON
        async def _make_request():
            # Use asyncio.to_thread to run the synchronous requests.post call in a separate thread
            # This prevents blocking the event loop if one is already running
            ...
            return response.json() # Call .json() on the response object

        # Run the async request in a new, isolated event loop
...
"You're still encountering the "Event loop is closed" error, and a new `AttributeError: 'coroutine' object has no attribute 'json'` error related to the Imagen API call. My apologies for the persistence of these issues.

Let's address these directly in the `app.py` Canvas.

The `AttributeError` occurs because `asyncio.to_thread` itself returns a coroutine, which needs to be `await`ed before you can call `.json()` on the `requests.Response` object it eventually yields. The "Event loop is closed" error is a tricky one in WSGI environments, and while `asyncio.run` creates a new loop, sometimes the underlying libraries (like `grpc` used by `google-generativeai`) still try to access a global or previously closed loop.

To make the API calls more robust in a synchronous Flask context, we will ensure that `asyncio.run` is used correctly with `asyncio.to_thread` for the `requests` call, and re-verify the structure for the `google.generativeai` call.

Here's the updated `app.py` Canvas with the fixes:
...
def generate_image_with_imagen(prompt):
    # The API key is automatically provided by Canvas if empty string
    apiKey = "" 
    apiUrl = f"[https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-002:predict?key=](https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-002:predict?key=){apiKey}"
    ...
    try:
        ...
        def _sync_request_and_json():
            ...
            response.raise_for_status() # Raise an exception for bad status codes (4xx or 5xx)
            return response.json() # Call .json() on the requests.Response object directly

        # Use asyncio.run and asyncio.to_thread to run the synchronous requests call
        # in a new thread, preventing event loop issues.
        result = asyncio.run(asyncio.to_thread(_sync_request_and_json))
...
def generate_daily_content():
    ...
    if not image_url:
        # Fallback to a placeholder if image generation fails
        if time_of_day == "morning":
            image_url = "[https://placehold.co/800x400/ADD8E6/000000?text=Morning+Inspiration](https://placehold.co/800x400/ADD8E6/000000?text=Morning+Inspiration)"
        elif time_of_day == "afternoon":
            image_url = "[https://placehold.co/800x400/90EE90/000000?text=Afternoon+Focus](https://placehold.co/800x400/90EE90/000000?text=Afternoon+Focus)"
        else: # evening
            image_url = "[https://placehold.co/800x400/8A2BE2/FFFFFF?text=Evening+Reflection](https://placehold.co/800x400/8A2BE2/FFFFFF?text=Evening+Reflection)"
